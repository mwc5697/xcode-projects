...
class WordModel {
    ...
    var randomCorrect : String {
        get {
            let correctAnswers = ["Great job!", "Perfect!", "You did it!"]
            let index = Int(arc4random_uniform(UInt32(correctAnswers.count)))
            return correctAnswers[index]
        }
    }
    
    var randomIncorrect : String {
        get {
            let incorrectAnswers = ["Maybe next time...", "Need to focus", "That's not it"]
            let index = Int(arc4random_uniform(UInt32(incorrectAnswers.count)))
